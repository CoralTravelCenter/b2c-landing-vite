...
async function processSection(sectionPath, config, cmsDir) {
  ...
  const cssPath = (cssRelPath && !/^https?:\/\//i.test(cssRelPath))
    ? path.resolve(path.dirname(sectionPath), cssRelPath)
    : null;
  const jsPath = (jsRelPath && !/^https?:\/\//i.test(jsRelPath))
    ? path.resolve(path.dirname(sectionPath), jsRelPath)
    : null;
